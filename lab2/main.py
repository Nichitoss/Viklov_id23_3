...
class Frog(QGraphicsEllipseItem):
    def __init__(self, x, y, weight, max_jump_distance):
        super().__init__(-15, -15, 30, 30)  # размеры лягушки
...
class LilyPad(QGraphicsEllipseItem):
    def __init__(self, x, y, strength):
        super().__init__(-25, -25, 50, 50)  # размеры кувшинки
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setFixedSize(800, 600)  # Установка фиксированного размера окна 1000x600
        self.scene = QGraphicsScene(0, 0, 800, 600)  # Создание сцены 800x600
        self.view = QGraphicsView(self.scene, self)
        self.view.setHorizontalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAlwaysOff)
        self.view.setVerticalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAlwaysOff)
        self.view.setGeometry(0, 0, 800, 600)  # Установка геометрии QGraphicsView

        # Переменные для управления
        self.trail_length = 5
        self.fall_speed = 1
        self.spawn_interval = 600
        self.lily_weight_max = 20
    ...
    def spawn_lily_pads(self):
        #таймер, который контролирует интервал появления новых кувшинок
        ...
        for _ in range(number_of_lilies):
            ...
            self.scene.addItem(lily)# Добавляем кувшинку в сцену с позицией (x, y)
    ...
    def update_frog_position(self):
        ...
        if next_position: # Найти кувшинку, c которой прыгнула лягушка
            jumped_lily = None
            for lily in self.lilies:
            ...
            if self.previous_lily_pad:
                if self.previous_lily_pad in self.lilies:  # Проверяем, что кувшинка всё ещё в списке
                    ...
                    if self.previous_lily_pad.strength <= 0:# Если прочность кувшинки достигла нуля, удаляем её из сцены
            ...
            for lily in self.lilies:
                if abs(lily.pos().x() - current_pos.x()) < 1:  # Если лягушка находится на кувшинке. Следуем за кувшинкой вниз
